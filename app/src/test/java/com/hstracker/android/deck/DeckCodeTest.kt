...
class DeckCodeTest {
    ...
    @Test
    fun `tollera blocco stile HSReplay con commenti e newline`() {
        val bytes = encodeDeck(
            format = 2,
            heroes = listOf(7),
            singles = emptyList(),
            doubles = listOf(1, 2, 3),
            nCopies = emptyList(),
        )
        val raw = Base64.getEncoder().encodeToString(bytes)
        // Blocco in stile "Copia da HSReplay": commenti, base64 spezzato su 2 righe, URL finale.
        val head = raw.substring(0, raw.length / 2)
        val tail = raw.substring(raw.length / 2)
        val block = """
            ### Il mio mazzo di prova
            # Class: Warrior
            # Format: Standard
            #
            # 2x (1) Something
            ...
            $tail
            #
            # To use this deck, copy it...
            https://hsreplay.net/decks/abcdef/
        """.trimIndent()

        val deck = DeckCode.decode(block)
        assertEquals(3, deck.cards.size)
        assertEquals(6, deck.totalCards)
    }
